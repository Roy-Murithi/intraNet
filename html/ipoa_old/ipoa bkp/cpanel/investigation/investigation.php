...
<table width="784" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text">
  ...
  <tr>
    <td width="413" height="22" valign="top">Investigations</td>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="Black_Header_Text"  >
	    ...
	    <tr>
	      <td width="36" height="24" valign="top" style="border-bottom:thin dotted;"><!--DWLayoutEmptyCell-->&nbsp;</td>
            ...
            <td width="417" valign="top" style="border-bottom:thin dotted;">Complaint</td>
		...
		$index=0;
		$rs=@mysql_query("select * from ".$pref."complaint where (`uneditable`='96' or `uneditable`='95' or `uneditable`='94' or `uneditable`='93')  and `complaintid` like '%$search%' order by `index` desc");
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=$start;$x<$end;$x++)
				{
					...
					<td valign=\"top\" style=\"border-bottom:thin dotted;\"><a href=\"#\" onclick=\"getPage('investigationdetails.php','content','complaintid=$data[0]&index=$start')\">Details</a>
					</td>

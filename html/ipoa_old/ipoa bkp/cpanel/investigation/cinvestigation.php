...
<table width="784" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text">
  ...
  <tr>
    <td width="444" height="22" valign="top">Concluded Investigations </td>
    <td colspan="2" valign="top"><div style="float:left">Search by Case Number:
      <input name="txtSearch" type="text" id="txtSearch" value="<?php $search=@$_GET['search'];echo @$search; ?>" />
        ...
        <?php 
	  $script="getPage('cinvestigation.php','content','sessid=smetsysmocmas&index=".(int)@$_GET['index']."&search=' +document.frmUsers.txtSearch.value)";
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="Black_Header_Text"  >
	    ...
	    <tr>
	      <td width="36" height="24" valign="top" style="border-bottom:thin dotted;"><!--DWLayoutEmptyCell-->&nbsp;</td>
            ...
            <td width="413" valign="top" style="border-bottom:thin dotted;">Complaint</td>
		...
		$next="Next";
		$prev="Previous";
		$url="cinvestigation.php";
		$index=0;
		$rs=@mysql_query("select * from ".$pref."complaint where (`uneditable`='94') and `complaintid` like '%$search%' order by `index` desc");
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=$start;$x<$end;$x++)
				{
					...
					$strDur="<div style=\"border:thin dotted; background:#FCFFBC;\">it has been ".getDuration(@$data[14],"since investigation was concluded") ."</div>";
					echo "
					<tr style=\"background:$color; border-bottom:thin dotted;\"><td valign=\"top\" style=\"border-bottom:thin dotted;\">".($x+1)."</td><td  valign=\"top\" style=\"border-bottom:thin dotted;\">$invno</td><td  valign=\"top\" style=\"border-bottom:thin dotted;\">$data[1]$strDur</td><td valign=\"top\" style=\"border-bottom:thin dotted;\">$inv<br /><font color=\"#FF0000\">$status</font></td>
					<td valign=\"top\" style=\"border-bottom:thin dotted;\"><a href=\"#\" onclick=\"getPage('cinvestigationdetails.php','content','complaintid=$data[0]&index=$start')\">Details</a>
					</td>

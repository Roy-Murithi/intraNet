...
<?php
 		//$rs=@mysql_query("select natureid,count(complaintid),EXTRACT(YEAR FROM to_date(`incident date`)) as year from ".$pref."complaint where 1 group by EXTRACT(YEAR FROM to_date(`incident date`))");
				...
				$legend[2]="Telephone";
				$legend[3]="Mail";
				$legend[4]="Social media";
				$legend[5]="Email";

				if(@$date1!="" && @$date2!="" )
				{
					$dat[0]=queryRecordCount("select  complaintid  from sm_main_complaint where (STR_TO_DATE(`incident date`,'%Y/%m/%d')>=STR_TO_DATE('$date1','%Y/%m/%d') and STR_TO_DATE(`incident date`,'%Y/%m/%d')<=STR_TO_DATE('$date2','%Y/%m/%d') ) and (complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[0]') or complaintid not in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[0]'))"); 
					$dat[1]=queryRecordCount("select  complaintid  from sm_main_complaint where (STR_TO_DATE(`incident date`,'%Y/%m/%d')>=STR_TO_DATE('$date1','%Y/%m/%d') and STR_TO_DATE(`incident date`,'%Y/%m/%d')<=STR_TO_DATE('$date2','%Y/%m/%d') ) and complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[1]')");
					$dat[2]=queryRecordCount("select  complaintid  from sm_main_complaint where (STR_TO_DATE(`incident date`,'%Y/%m/%d')>=STR_TO_DATE('$date1','%Y/%m/%d') and STR_TO_DATE(`incident date`,'%Y/%m/%d')<=STR_TO_DATE('$date2','%Y/%m/%d') ) and complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[2]')");
					$dat[3]=queryRecordCount("select  complaintid  from sm_main_complaint where (STR_TO_DATE(`incident date`,'%Y/%m/%d')>=STR_TO_DATE('$date1','%Y/%m/%d') and STR_TO_DATE(`incident date`,'%Y/%m/%d')<=STR_TO_DATE('$date2','%Y/%m/%d') ) and complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[3]')");
				...
				{
					$dat[0]=queryRecordCount("select  complaintid  from sm_main_complaint where complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[0]') or  complaintid not in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[0]'))");
					$dat[1]=queryRecordCount("select  complaintid  from sm_main_complaint where complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[1]')");
					$dat[2]=queryRecordCount("select  complaintid  from sm_main_complaint where complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[2]')");
					$dat[3]=queryRecordCount("select  complaintid  from sm_main_complaint where complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[3]')");
					$dat[4]=queryRecordCount("select  complaintid  from sm_main_complaint where complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[4]')");
					$dat[5]=queryRecordCount("select  complaintid  from sm_main_complaint where complaintid in (select complaintid from sm_main_complaintdetails where lodgemode='$legend[5]')");
				}
        ...
        $('#containerx').highcharts({
            ...
            title: {
                text: 'Summary report on complaint lodging mode'
            },
            subtitle: {
            ...
            xAxis: {
				...
				categories:  <?php echo "['Lodging mode']"; ?>
            },
            yAxis: {
                title: {
            ...
            series: [{
                ...
                name: '<?php echo $legend[5]; ?>',
                data: <?php echo "[".$tempData[5]."]"; ?>
            }]
        });
    });
...
<table width="801" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text" style="border-bottom:thin solid ">
  ...
  <tr>
    ...
    <td width="788" valign="top"><span class="style15">Summary report on complaint lodging mode </span> </td>
  </tr>
</table>
...
<table class="rptTable Black_Header_Text" width="801" border="0" cellpadding="0" cellspacing="0" style="border:thin solid;">
  ...
  <?php
 		//$rs=@mysql_query("select natureid,count(complaintid),EXTRACT(YEAR FROM to_date(`incident date`)) as year from ".$pref."complaint where 1 group by EXTRACT(YEAR FROM to_date(`incident date`))");
		echo "<tr ><td>$legend[0]</td><td>$legend[1]</td><td>$legend[2]</td><td>$legend[3]</td><td>$legend[4]</td><td>$legend[5]</td><td>Total</td></tr>";				
        echo "<tr><td>$dat[0]</td><td>$dat[1]</td><td>$dat[2]</td><td>$dat[3]</td><td>$dat[4]</td><td>$dat[5]</td><td>".((int)$dat[0]+(int)$dat[1]+(int)$dat[2]+(int)$dat[3]+(int)$dat[4]+(int)$dat[5])."</td></tr>" ;        

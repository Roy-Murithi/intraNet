...
/*		if($statusv=="98")
		...
		{*/
			$rs=@mysql_query("select * from ".$pref."complaint where (`uneditable`='93') and `complaintid` like '%$search%' order by `index` desc");
		//}
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=$start;$x<$end;$x++)
				{
					...
					$strDur="<div style=\"border:thin dotted; background:#FCFFBC;\">it has been ".getDuration(@$data[16],"since investigation was Finalized") ."</div>";
					echo "

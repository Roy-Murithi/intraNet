...
if(@$_GET["pwardid"]!="")
{
	...
	{
		$rs=@mysql_query("select * from ".$pref."pward where `pwardid`='".@$_GET['pwardid']."'");
		if($rs)
		{
...
<table width="717" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text">
  ...
  <tr>
    ...
    <td colspan="6" valign="top">Edit ward </td>
    <td width="17">&nbsp;</td>
  </tr>
  <tr>
    <td height="23">&nbsp;</td>
		...
		if($rs)
		{
			...
			if ($counts>0)
			{
				for($x=0;$x<$counts;$x++)
				{
					...
					echo "<option id=\"$data[1]\" $selected >$data[1]</option>\n";
				}
			}
		}
	?>
  ...
  <tr>
    ...
    <td colspan="2" valign="top"><input name="txtDat1" type="text" class="STR1" id="txtDat1" value="<?php echo @$datax[1]; ?>" />
      <input name="pwardid" type="hidden" id="pwardid"  value="<?php echo @$datax[0]; ?>"/></td>
    <td></td>
    <td></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	{
		echo "<input type=\"button\" name=\"add\" value=\"Add new Ward\"  class=\"BTN\" onclick=\"saveUser()\">";
	}
	?></td>
    <td valign="top"><input name="Button2" type="button" class="BTN" value="Cancel" style="width:100px;" onclick="getPage('ward.php','content','')" /></td>
    <td></td>
    <td></td>
  </tr>
  <tr>

...
if($galleryid!="" && $galleryid!="undefined" && $galleryid!=NULL && $postid!="" && $postid!="undefined" && $postid!=NULL )
{
		$imgFile="";
	...
	if("../../".$imgFile!="")
	{
		$rs=@mysql_query("select * from `".$pref."post` where `postid`='$postid'");
		if($rs)
		{
			$counts=@mysql_num_rows($rs);
			if ($counts>0)
			{
				...
				/*if(is_file("../../$data[6]")!="")
				{
					...
					/*if((int)$datH>0 && (int)$datW>0 )	
					{
						...
						//echo "imagecopyresampled( $dst_img,$src_img,0,0,$datL,$datT,200,200,$datW,$datH);";exit;
						...
						if(strtolower(substr($imgFile,strlen($imgFile)-4,4))==".jpg")
						{
							...
							$query="update `".$pref."post` set `thumbnail`='gallery/thumb/$data[0]_thumb.jpg' where `postid`='$postid'";
						}elseif(strtolower(substr($imgFile,strlen($imgFile)-4,4))==".png")
						{
							imagepng($dst_img, "../../gallery/thumb/$data[0]_thumb.png" );
							$query="update `".$pref."post` set `thumbnail`='gallery/thumb/$data[0]_thumb.png' where `postid`='$postid'";
						}*/
						$query="update `".$pref."post` set `thumbnail`='$imgFile' where `postid`='$postid'";
						mysql_query($query);

...
<table width="659" class="Black_Header_Text">
	      <!--DWLayoutTable-->
            <tr>
              <td height="20" colspan="3" valign="top"> Post images </td>
  ...
  <tr>
		...
		$rs=@mysql_query("select * from `".$pref."gallery` where `category`='post'");
		if($rs)
		{
			$counts=@@mysql_num_rows($rs);
			if ($counts>0)
			{
				...
				if($offset>0)
				{
					$First="<a href=\"#\" onclick=\"getPage('listimages.php','content','index=0&postid=$postid')\">First</a>";
					if($offset-$maxi<0)
					{
						$prev=0;
					}else
					...
					$Previous="<a href=\"#\" onclick=\"getPage('listimages.php','content','index=$prev&postid=$postid')\">Previous</a>";
				}else
				{
					$First="First";
					$Previous="Previous";
				...
				if($offset+$max<$counts)
				{
					...
					$nex=$offset+$max;
					$Next="<a href=\"#\" onclick=\"getPage('listimages.php','content','index=$nex&postid=$postid')\">Next</a>";
				}else
				{
					$Last="Last";
					$Next="Next";
				...
				{	@mysql_data_seek($rs,$x);
					...
					{
						$editors=" <a href=\"inserter.php?galleryid=$data[0]&postid=$postid&sessid=smetsysmocmas\">Insert into post</a> | <a href=\"uploader.php?galleryid=$data[0]&postid=$postid&sessid=smetsysmocmas\">Edit</a> | ";
					}else
					{
						$editors="";
					}
					if($postid!="")
				{
					$fimage="<a href=\"featured.php?galleryid=$data[0]&sessid=smetsysmocmas&postid=$postid\">Use as featured image</a> | ";
				}else
				{
					$fimage="";
				}
					...
					<div class=\"loader\" align=\"left\" style=\"width:653px;height:70px;float:left;display:block;border-bottom:thin dotted;\" onClick=\"getPage('uploader.php','','galleryid=$data[0]')\">
						...
						<div style=\"width:350px;float:left;\" align=\"right\">
							$fimage $editors<a href=\"listimages.php?del=99&galleryid=$data[0]&postid=$postid&sessid=smetsysmocmas\">Delete</a>
						</div>
					</div>

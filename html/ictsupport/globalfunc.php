<?php 
	if(@$_SESSION['loggedIn']!=990 && @$ignore_login!=true)
	{
		header("location:../index.php?pid=0");
		exit;
	}
function toSentenceCase($strNames)
{
	$temp=array();
	$temp=explode(" ",$strNames);
	for($x=0;$x<sizeof($temp);$x++)
	{
		$temp[$x]=strtoupper(substr($temp[$x],0,1)).strtolower(substr($temp[$x],1, strlen($temp[$x])-1));
	}
	return join(" ",$temp);
}
...
function getDays($end)
{
		...
		if(strpos($end,"/") )
		{
			...
			if((int)@$dtemp[1]<=12 && (int)@$dtemp[2]<=31)
			{
				$intervalo = @date_diff( date_create($end),date_create(date("Y/m/d H:i:s")));
				$out="";
				//$out = $intervalo->format("Years:%Y,Months:%M,Days:%d,Hours:%H,Minutes:%i,Seconds:%s");
				$out=(int)$intervalo->format("%R%a");
			}
		}
        return $out;		      
}

function isCleared($cleared)
{
	if(strpos(" ".$cleared,$_SESSION['level'])>0)
	{
		return true;
	}else
	{
		header("location:/mcp/noclearance.php");
		return false;
	}
}
...
function _getDirectoryUsername($userid)
{
	$names="Unknown";
	$rs=@mysql_query("select names from person where `personid`='$userid'");
	if($rs)
	{
	...
	if($names=="Unknown")
	{ 
		$rs=@mysql_query("select names from sm_main_user where `userid`='$userid'");
		if($rs)
		{

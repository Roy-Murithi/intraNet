  ...
  <tr>
    <td height="31" colspan="4" valign="top">
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="Black_Header_Text"  >
		...
		$url="complaint.php";
		$index=0;
		$rs=@mysql_query("select * from ".$pref."cfilecheck  order by `check` desc");
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=$start;$x<$end;$x++)
				{
					...
					<td valign=\"top\" style=\"border-bottom:thin dotted;\"><a href=\"#\" onclick=\"getPage('editfilecheck.php','content','cfilecheckid=$data[0]&index=$start')\">Edit</a></td>

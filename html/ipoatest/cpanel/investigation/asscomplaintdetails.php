...
{?>
	<html>
		...
		<script language="javascript">
			...
			getPage("assigninvestigation.php","content","index=<?php echo @index;?>");
		</script>
	</html>
<?php
	exit;
		...
		for($x=0;$x<sizeof($temp);$x++)
		{
			...
			if($rs1)
			{
				...
				if ($counts1>0)
				{
					...
					{
					$div=$div."<div style=\"float:left;position:relative;\" ><a class=\"flink\" href=\"viewpersons.php?url=asscomplaintdetails&personsid=$datap[0]&complaintid=$datax[0]&sessid=smetsysmocmas&index=$index\"><div id=\"persons\" class=\"persons\" style=\" border: thin solid #7Ba89A; margin-left:20px;margin-top:2px;margin-bottom:2px;  padding-top:3px; padding-bottom:3px; padding-left:10px; padding-right:5px;\">$datap[2]. $datap[3] $datap[4] $datap[5] $onbehalfname</div></a></div>";
					}
				}
			}
		}
...
-->
 ...
 <table style="margin-left:15px;"  width="728" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text">
  <!--DWLayoutTable-->

  
  
  <tr>
    <td height="22" colspan="4" valign="top">Assignment of Investigation by head/deputy director of investigations</td>
    <td colspan="5" valign="top"><div align="right" style="font-size:20px;"><?php echo "Complaint file N<u>o</u>: ".@$datax[0];?></div></td>
    <td width="1"></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	if($invid=="-"){$invid="";}
		if($invid!="")
		{
			$script="getPage('return1.php','content','complaintid=$datax[0]&sessid=smetsysmocmas&index=$index&pid=assigninvestigation&pid1=asscomplaintdetails&uneditableVal=99&rtitle=complaint to complaints department&complaintfield=back_to_approval')"; 
		}else
		{
			$script="alert('You must select lead investigator');";
		}
		echo classBTN("btnReturn","Open investigation","#","","$script","#FF0000"); 
		?>		</td>
    <td colspan="2" rowspan="2" valign="top"><?php
		$script="getPage('return.php','content','complaintid=$datax[0]&sessid=smetsysmocmas&index=$index&pid=assigninvestigation&pid1=asscomplaintdetails&uneditableVal=99&rtitle=complaint to complaints department&complaintfield=back_to_approval')"; 
		echo classBTN1("btnReturn","Return to Complaints","#","","$script","#FF0000"); 
		?></td>
    <td></td>
  </tr>
  <tr>
    <td colspan="4" rowspan="2" valign="top">Complaint  (<?php echo @$datax[26];?>) </td>
    <td height="10"></td>
  </tr>
  ...
  <tr>
    <td height="65"></td>
    <td colspan="7" valign="top" style="background-color:#ABD8DA; border: thin solid #7Ba89A"><?php echo @$datax[1];?></td>
    <td valign="top"><?php echo getUrgent($pref,$datax[0],25,60);?></td>
    <td></td>
  </tr>
  <tr>
    <td height="9"></td>
  ...
  <tr>
    <td height="47"></td>
    <td valign="top"><div align="right">Lead Investigator:</div></td>
    <td>&nbsp;</td>
    <td colspan="2" valign="top" style="background-color:#ABD8DA; border: thin solid #7Ba89A"><?php 
	$invid=fetchValue($pref."investigation","complaintid",$complaintid,2);
    ...
    <td colspan="3" valign="top"><div align="left">
      <?php
		if(fetchRecordCount($pref."investigation","complaintid",$complaintid)==0)
		{
			$btnCaption="Assign lead Investigators";
			$sval=0;
		}else
		{
			$btnCaption="Swap lead Investigators";
			$sval=99;
		}
		$script="getPage('attachinv.php','content','complaintid=$datax[0]&sessid=smetsysmocmas&index=$index&leadinv=99&swap=$sval');"; 
		echo classBTN("btnApprove","$btnCaption","#","","$script"); 
		?>
    </div></td>
    <td></td>
  </tr>
  ...
  <tr>
    <td height="18"></td>
    <td valign="top"><div align="right">Support Investigatos:</div></td>
    <td></td>
    <td colspan="2" rowspan="2" valign="top" style="background-color:#ABD8DA; border: thin solid #7Ba89A">
	<?php 
		...
		for($x=0;$x<sizeof($temp);$x++)
		{
			if($temp[$x]!="")
			{
				...
				if($invnames!="")
				{
					...
					$pic="../../".$img;?>" border="1" style="border-color:B2D1B2" height="<?php echo getPicH($pic,20); ?>" width="<?php echo getPicW($pic,20); ?>"  /><?php
					echo "</div><div style=\"float:left\">$invnames</div>&nbsp;";
					$script="getPage('saveattachinv.php','content','invid=$temp[$x]&complaintid=$complaintid&sessid=smetsysmocmas&action=0&index=$index&del=99')"; 
					echo "<div style=\"float:left\">".classBTN("btnApprove","<img src=\"../images/ico_drop.png\">","#","","$script")."</div></div>";
					}
				}
			}
		}
    ...
    <td colspan="3" valign="top"><?php
		$script="getPage('attachinv.php','content','complaintid=$datax[0]&sessid=smetsysmocmas&index=$index&leadinv=0');"; 
		echo classBTN("btnApprove","Attach supporting Investigators","#","","$script"); 
		?></td>
    <td></td>
  </tr>
  <tr>
  ...
  <tr>
    <td height="6" colspan="9" valign="top"><img src="../images/phead.png" width="700" height="5" /></td>
    <td></td>
  </tr>
</table>

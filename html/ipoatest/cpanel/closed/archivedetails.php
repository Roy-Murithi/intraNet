...
if(@$_GET["complaintid"]!="")
{
	$rs=@mysql_query("select * from ".$pref."archive where `complaintid`='".@$_GET['complaintid']."'");
	if($rs)
	{
		...
		for($x=0;$x<sizeof($temp);$x++)
		{
			...
			if($rs1)
			{
				...
				if ($counts1>0)
				{
					...
					{
						$div=$div."<div style=\"float:left;position:relative;\" ><a class=\"flink\" href=\"viewpersons.php?url=archivedetails&personsid=$datap[0]&complaintid=$datax[0]&sessid=smetsysmocmas&index=$index\"><div id=\"persons\" class=\"persons\" style=\" border: thin solid #7Ba89A; margin-left:20px;margin-top:2px;margin-bottom:2px;  padding-top:3px; padding-bottom:3px; padding-left:10px; padding-right:5px;\">$datap[2]. $datap[3] $datap[4] $datap[5] $onbehalfname</div></a></div>";
					}
				}
			}
		}
  ...
  <tr>
    ...
    <td colspan="2" valign="top"><div align="right">
      <?php
		...
		$script="getPage('saveunarchive.php','content','complaintid=$datax[0]&sessid=smetsysmocmas&index=$index')"; 
		echo classBTN1("btnReturn","Unarchive this investigation","#","","$script","#FF0000"); 
		?>
    </div></td>

...
<table width="784" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text">
  ...
  <tr>
    <td height="22" colspan="2" valign="top">Occurence</td>
    <td width="167" valign="top"><div align="right"><?php
		$script="";
		echo classBTN("btnReturn","Add new occurence","editobcomplaint.php","sessid=smetsysmocmas","$script","#FF0000"); 
		?></div></td>
    </tr>
  <tr>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="Black_Header_Text"  >
	    ...
	    <tr>
	      <td width="36" height="24" valign="top" style="border-bottom:thin dotted;"><!--DWLayoutEmptyCell-->&nbsp;</td>
            <td width="110" valign="top" style="border-bottom:thin dotted;">Occurence file N<u>o</u> </td>
            <td width="379" valign="top" style="border-bottom:thin dotted;">Occurence</td>
		...
		$url="obcomplaint.php";
		$index=0;
		$rs=@mysql_query("select * from ".$pref."OB  where (`uneditable`='99')  order by  `index` desc");
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=$start;$x<$end;$x++)
				{
					...
					$func="getPage('sobcomplaintdetails.php','content','OBid=$data[0]&index=$start&url=sobcomplaint')";
					$strDur="<div style=\"border:thin dotted; background:#FCFFBC;\">it has been ".getDuration(@$data[8],"since occurence was reported") ."</div>";
					$strStatus= getUrgent($pref,$data[0],25,40);					
					echo "
					<tr style=\"background:$color; border-bottom:thin dotted;\"><td valign=\"top\" style=\"border-bottom:thin dotted;\">".($x+1)."</td><td valign=\"top\" style=\"border-bottom:thin dotted;\">$data[0]</td><td  valign=\"top\" style=\"border-bottom:thin dotted;\">$data[5]$strDur</td>
					
					<td valign=\"top\" style=\"border-bottom:thin dotted;\">$data[8]</td>
					<td valign=\"top\" style=\"border-bottom:thin dotted;\"> <a href=\"#\" onclick=\"$func\">View occurence</a>
					</td>

...
if(@$_GET["activate"]=="99")
	{
		if(@$_GET["actvt"]=="0"){$actvt="0";}else{$actvt="99";}
		$rs=@mysql_query("update meetings set `active`='$actvt' where `meetingsid`='".@$_GET['meetingsid']."'");		
	}
?>
...
<table width="784" border="0" cellpadding="0" cellspacing="0" class="Black_Header_Text">
  ...
  <tr>
    <td height="32" colspan="2" valign="top">Activate meetings schedule</td>
		...
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=$start;$x<$end;$x++)
				{
					...
					if($data[7]=="0")
					{
						$flink="<a href=\"#\" onclick=\"getPage('activate.php','content','meetingsid=$data[0]&index=$start&activate=99&actvt=99')\">Activate</a>";
					}else
					{
						$flink="<a href=\"#\" onclick=\"getPage('activate.php','content','meetingsid=$data[0]&index=$start&activate=99&actvt=0')\">Deactivate</a>";
					}
					echo "
					<tr style=\"background:$color; border-bottom:thin dotted;\"><td valign=\"top\" style=\"border-bottom:thin dotted;\">".($x+1)."</td><td valign=\"top\" style=\"border-bottom:thin dotted;\">$data[1]</a></td><td valign=\"top\" style=\"border-bottom:thin dotted;\">$data[4]</a></td><td valign=\"top\" style=\"border-bottom:thin dotted;\">
					$flink
					</td>

...
<table width="794" class="Black_Header_Text">
	      <!--DWLayoutTable-->
            <tr>
              <td width="352" height="31" valign="top">Staff</td>
		...
		$script="";
		echo classBTN("btnReturn","Add Staff","editstaff.php","sessid=smetsysmocmas","$script","#FF0000"); 
		?></div></td>
  <tr>
		...
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				if(@$data[9]=="99"){$levelName="Undefined";}
		   		elseif(@$data[9]=="0"){$levelName="Director - Business Services";}
		 		elseif(@$data[9]=="1"){$levelName="Director - Investigations";}
		  		elseif(@$data[9]=="2"){$levelName="Director - Complaints & Legal";}
		   		elseif(@$data[9]=="3"){$levelName="Director - Research";}
		  		elseif(@$data[9]=="4"){$levelName="Director - Monitoring and Evaluation";}
				...
				elseif(@$data[9]=="11"){$levelName="Records Management Officer";}
		  ...
		  ?>" border="1" style="border-color:B2D1B2" height="<?php echo getPicH($pic,100); ?>" width="<?php echo getPicW($pic,100); ?>"  />	
						...
						<div  style=\" width:180px; height:100px;\"  align=\"center\">
							...
							<a href=\"#\" onclick=\"delPage('staff.php','content','staffid=$data[0]&del=99&index=$offset')\">Delete</a><br />
							<a href=\"#\" onclick=\"getPage('../accessrights/accessrights.php','content','staffid=$data[0]')\">Access rights</a> 
						</div>
					</div>				  
				  	</div>
					";

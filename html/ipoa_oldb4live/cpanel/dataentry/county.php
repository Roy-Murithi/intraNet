...
$facultyid=@$_GET['countyid'];
if(@$_GET["countyid"]!="")
{
	if(@$_GET["del"]=="99")
	{
		$rs=@mysql_query("delete from ".$pref."county where `countyid`='".@$_GET['countyid']."'");
	}
}
...
<form action="savecounty.php" method="post" enctype="multipart/form-data" name="frmUsers">
...
<table width="644" border="0" cellpadding="0" cellspacing="0" class="PlainContent_Box">
  ...
  <tr>
    <td width="529" height="22" valign="top">Edit County </td>
    <td width="167" valign="top"><div align="right"><?php
		$script="";
		echo classBTN("btnReturn","Add new County","editcounty.php","sessid=smetsysmocmas","$script","#FF0000"); 
		?></div></td>
    <td width="4">&nbsp;</td>
  </tr>
  <tr>
		...
		if($rs)
		{
			...
			if ($counts>0)
			{
				...
				for($x=0;$x<$counts;$x++)
				{
					...
					<td><a href=\"#\" onclick=\"getPage('editcounty.php','content','countyid=$data[0]')\">Edit</a> | <a href=\"#\" onclick=\"delPage('county.php','content','countyid=$data[0]&del=99')\">Delete</a></td>
					</tr>
					";
				}
			}

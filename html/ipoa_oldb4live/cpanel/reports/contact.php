  ...
  <tr>
    <td height="44" colspan="3" valign="top">
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="Black_Header_Text"  >
		...
		$rs=@mysql_query("select * from ".$pref."persons where `surname` like '%$search%' or `firstname` like '%$search%' or `lastname` like '%$search%' or `station` like '%$search%'   or `custodystation` like '%$search%' or `representation_reason` like '%$search%'order by `surname` asc");
		if($rs)
		{
